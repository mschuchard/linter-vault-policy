  ...
  provideLinter() {
    return {
      ...
      lint: async (textEditor) => {
        ...
        if (!(/path ".*"/.exec(textEditor.getText()))) return [];
        ...
        return helpers.exec(atom.config.get('linter-vault-policy.vaultExecutablePath'), ['policy', 'fmt', file], { stream: 'stderr', env: [['VAULT_CLI_NO_COLOR', 'true']], allowEmptyStderr: true }).then(output => {
          ...
          if (matchesError != null) {
            toReturn.push({
              ...
              location: {
                file,
                position: [[Number.parseInt(matchesError[1], 10) - 1, Number.parseInt(matchesError[2], 10) - 1], [Number.parseInt(matchesError[1], 10) - 1, Number.parseInt(matchesError[2], 10)]],
              },
            });
          }
          return toReturn;
