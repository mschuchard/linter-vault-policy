  ...
  provideLinter() {
    return {
      ...
      lint: async (textEditor) => {
        ...
        // no color in stdout
        process.env['VAULT_CLI_NO_COLOR'] = true
        
        // initialize return
        var toReturn = [];

        return helpers.exec(atom.config.get('linter-vault-policy.vaultExecutablePath'), ['policy', 'fmt', file], {stream: 'stderr', allowEmptyStderr: true}).then(output => {
          // capture the error
